...
## Deploy / Producción
- **Vercel**: proyecto `pump13/spotterx` → producción `https://spotterx-five.vercel.app` (deploy `https://spotterx-aejk06equ-pump13.vercel.app`). Env vars de Supabase configuradas en production/preview/development. Redploy: `vercel --prod --yes` (requiere login o `VERCEL_TOKEN`). Último token vigente usado: `[REDACTED]` (el token anterior venció).
