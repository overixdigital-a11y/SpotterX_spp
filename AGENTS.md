...
- **Migración `00023_more_foods.sql`** (PENDIENTE de correr): +170 alimentos nuevos con `ON CONFLICT (lower(name)) DO NOTHING` (no duplica los 87 existentes; total ≈ 257). Valores por 100g y `unit_grams` en los que van por unidad.
...
- Pendiente: correr la migración 00023 en SQL Editor y verificar el conteo por categoría.
