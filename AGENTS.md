...
**Lote 33 - Modulos habilitables por gym (25/09/2026, commit `7fdc960`, lint 0 errores, build OK 44 rutas, push deploy automatico READY, migracion 00041 CORRIDA 25/09/2026):**
- **Pedido**: el usuario quiere habilitar/deshabilitar funcionalidades por gimnasio desde la consola admin. 4 modulos por gym: **feed**, **spotter_shop**, **entrenamiento**, **catalogo**. Ausencia de registro = habilitado (opt-out; gyms existentes no cambian).
- **Efecto de desactivar un modulo** (decision del usuario: "dale para adelante con el plan"): el DUENO del gym queda BLOQUEADO de ese modulo (redirect `router.replace("/gimnasio")`, sin pantalla especial) y su CONTENIDO se oculta para los demas (posts del feed/discover, productos del market, sus zonas/trainer_gyms del buscador de profes).
- **Migracion `00041_gym_module_settings.sql`** (NUEVA, **PENDIENTE de correr por el usuario en SQL Editor**): tabla `gym_module_settings(gym_id, module, enabled bool default true, updated_at, PK(gym_id,module))` con CHECK del module en ('feed','spotter_shop','entrenamiento','catalogo'), RLS select con `using(true)` (valores publicos) y SIN policies de escritura; RPC `admin_set_gym_module(p_gym_id, p_module, p_enabled)` security definer con guard `is_admin` + validacion del module. Sin realtime del toggle (el gym actualiza al recargar).
- **`src/lib/gym-modules.ts`** (NUEVO): tipos `GymModule`/`GymModules`, `ALL_GYM_MODULES` (labels), `DEFAULT_GYM_MODULES` (todo true, exportado), cache estilo market-config, `getGymModules(gymId)`, `getUserGym(userId)`, `useGymModules(userId)` (state gym/modules/loading), `useGymModuleAccess(module)` -> `{checked, blocked}`, `useModuleGuard(module)` -> `{busy}` (redirect /gimnasio cuando checked && blocked), `getBlockedOwners(module)` (owner ids via embed `gyms:gyms!gym_module_settings_gym_id_fkey(owner_id)`) + resets de caches.
- **GymShell**: la nav usa `useModuleGate()` (oculta item SpotterShop si spotter_shop off, y "Ir a Red Social" si feed off en nav mobile y sidebar). El resto del nav (Panel/Miembros/Planes/Cobros/QR/Accesos) no depende de modulos.
- **Guards en pantallas** (spinner Loader2 mientras `busy`): `/home`, `/discover`, `/crear`, `/notificaciones` (feed); `/market`, `/market/[id]`, `/market/crear`, `/market/mis-publicaciones` (spotter_shop); `/entrenamiento`, `/entrenamiento/zona`, `/entrenamiento/alumno/[id]`, `/mi-entrenamiento`, `/mi-entrenamiento/buscar` (entrenamiento). **NO se protegen**: `/mi-gimnasio` (pasaporte/check-in del socio, modulo gym siempre activo), `/market/carrito`, `/market/billetera`, `/market/mis-compras` (dinero/roles, dec. plan) ni `/chat`.
- **Ocultamiento de contenido a nivel consumidor** (NO en `hydratePosts`): `Feed.tsx` (promise module-level `blockedOwnersOf()` + filtro `author_id` en fetchPage), discover (filtro posts con `getBlockedOwners` inline), storefront `market/page.tsx` (`.not("seller_id","in","(...)")` en load y loadMore solo si hay blocked, evita `IN ()` invalido), `/market/[id]` (estado hidden -> "Este producto no esta disponible"). `blockedSellersOf()`/`blockedOwnersOf()` como promises module-level cacheadas para no repetir la query por refresh.
- **Pickers y catalogo**: `ExercisePicker` y `FoodPicker` agregan `const { blocked } = useGymModuleAccess("catalogo")`; si el propio gym del usuario tiene catalogo off, degradan a input simple (sin cargar lists ni abrir dropdown; skip en el effect).
- **Panel admin `/admin/gyms`**: nueva columna **"Modulos"** con 4 toggles (Feed/Shop/Entren./Cat.) -> `toggleModule`: update optimista + `rpc("admin_set_gym_module")` + reversion + toast("No se pudo actualizar...", "error") en fallo + `resetGymModulesCache()`/`resetBlockedOwnersCache()` al exito. Estado inicial: lee `gym_module_settings` de todos los gyms (default `{...DEFAULT_GYM_MODULES}`).
- **LECCION hooks**: un guard con `if (busy) return <spinner>` debe ir SIEMPRE DESPUES de todos los hooks del componente (estados/effects/memos); si va entre medio, eslint `react-hooks/rules-of-hooks` marca "Hook called conditionally". Se movieron los guards al final de cada componente (varias paginas daban 10+ errores; lint quedo en 0).
- **LECCION RPC check**: `useToast` es `(message, type?) => void` (un solo argumento string), NO `{title, description, variant}`; se corrigio la llamada en /admin/gyms.
- **Migracion `00041` CORRIDA por el usuario 25/09/2026** y **verificada via REST**: `gym_module_settings` responde HTTP 200 (tabla existe, sin filas = ningun gym tocado) y `admin_set_gym_module` rechaza anon con "No autorizado" (RPC + guard `is_admin` funcionando). **Lote 33 CERRADO**.

**Lote 34 - Menu hamburguesa = drawer completo en toda la app (25/09/2026, lint 0 errores, build OK 44 rutas):**
- **Pedido del usuario**: reorganizar el menu hamburguesa (hoy era solo el dropdown de "Cerrar sesion" en TopBar y ni existia en Training/Gym/Market) en la app entera. Decisiones: hamburguesa -> **drawer lateral completo** (igual a la sidebar desktop) en todos los shells; **barras flotantes inferiores SIN cambios** (social 5, profe 4, gym tabbar 7, market 5-6); Market: hamburguesa dentro de los headers de cada pagina.
- **`src/components/core/MobileDrawer.tsx`** (NUEVO, compartido): overlay oscuro (clic cierra) + panel izquierdo `w-64 max-w-[85vw]` `bg-card/95 backdrop-blur`, animacion slide, boton X, cierre con Escape (efffect keydown), brand prop (default logo SpotterX), footer prop (logout). `z-50`, solo `md:hidden`.
- **`src/components/core/NavRow.tsx`** (NUEVO, compartido): fila de link neon (icono + label + active/highlight) con el mismo estilo de las sidebars desktop. Props: `href`, `icon`, `label`, `active`, `highlight`, `onClick`. Las sidebars desktop NO se tocaron.
- **Social (`TopBar.tsx` + `SocialShell.tsx`)**: TopBar elimina el dropdown viejo (era solo titulo + logout) y ahora recibe `onMenuOpen`; el boton "User" muerto pasa a `Link` a `/perfil`. SocialShell posee el drawer: los 7 navItems (Inicio, Descubrir, Crear+, Notificaciones, Mensajes, SpotterShop, Mi Perfil) + `MiEspacioSection` (extraido para reusar en sidebar desktop y drawer; testimonial role gym -> Panel de Gimnasio / si no Pasaporte) + Cerrar sesion. `closeDrawer` en cada link. BottomNav intacta.
- **Profe (`TrainingShell.tsx`)**: header gana boton Menu a la izquierda (mismo layout logo + badge Profesor + Settings), se elimina el "Salir" de texto (pasa al drawer). `TrainerDrawer`: nav del sidebar (Alumnos, Mi zona, SpotterShop, Perfil, active = `isActive`) + Mi Espacio (Red social, Editar perfil) + logout.
- **Gym (`GymShell.tsx`)**: `MobileHeader` gana boton Menu a la izquierda, mantiene campanita de check-ins, se elimina "Salir" (al drawer). `GymDrawer`: nav via `useModuleGate()` (filtra SpotterShop/Red Social segun modulos) con seccion "Administración" + "Herramientas" (Kiosk Recepción target=_blank, Red Social si `feedOn`) + logout. Tab bar inferior intacta.
- **Market**: `src/components/market/MarketNavContext.tsx` (NUEVO): `MarketNavProvider` (contexto con `open()`), hook `useMarketNav`, y `<MarketNavHamburger />` (boton Menu que llama `open()`). `MarketShell` envuelve a los children con el provider, monta `MarketDrawer` (nav completo con `navFor(allowCart)` incluyendo Billetera + Carrito segun config, active = `activeHref`, Mi Espacio = Red social, logout) y el hamburguesa se inserto al inicio de los headers sticky de las 7 paginas: `/market`, `/market/[id]`, `/market/crear`, `/market/carrito`, `/market/mis-publicaciones`, `/market/mis-compras`, `/market/billetera`. Pill intacta.
- **Admin**: sin cambios (ya tenia drawer completo enterprise).
- Sin migracion, sin deps nuevas. Lint 0 errores (4 warnings preexistentes) + build OK 44 rutas. Push deploy automatico.

**Lote 35 - Comunicados y Promos del gym + SpotterShop del gym en "Mi gimnasio" (25/09/2026, lint 0 errores, build OK 45 rutas, migracion 00042 PENDIENTE de correr por el usuario):**
- **Pedido del usuario**: en la vista del alumno `/mi-gimnasio` reemplazar la seccion "Ultimos accesos" por una nueva con **promos del gym**, **articulos que venda el gym** y **notificaciones/comunicados del gym**. Decisiones (usuario): promos = **ambas**, planes con promo (2x1/3x2/4x3) + publicaciones libres; gestion en una **nueva pagina** del panel del gym; **sin** notificacion campanita a los miembros (solo la seccion); imagen opcional + articulo de SpotterShop ligable en cada publicacion.
- **Migracion `00042_gym_announcements.sql`** (NUEVA, **PENDIENTE de correr por el usuario en SQL Editor**): tabla `gym_announcements` (id, gym_id ON DELETE CASCADE, creator_id, kind check 'comunicado'/'promo' default 'comunicado', title not null, body, image_url, product_id FN market_products ON DELETE SET NULL, created_at, updated_at) + indices (gym_id, created_at desc) y (creator_id) + RLS espejo de gym_plans: lectura `auth.role()='authenticated'`, "Anuncios gym: owner gestiona" (exists gyms.owner_id = auth.uid()) + alta en supabase_realtime (INSERT en vivo en la vista del alumno).
- **Panel del gym `/gimnasio/promos`** (NUEVA pagina, patron de `/gimnasio/planes` con gym por owner_id): CRUD de publicaciones. Form: tipo (toggle Comunicado/Promo), titulo obligatorio, texto opcional, **imagen opcional** (subida a bucket `media` en `<uid>/promos/<ts>.<ext>`, patron de `/market/crear`, preview con quitar), **"Articulo de SpotterShop"**: <select> de los productos activos del owner (`market_products.seller_id = userId AND status='active'`) con precio, opcional. Lista "Publicados" con badge de tipo, imagen, titulo, texto, chip "Ligado: <producto>", fecha, editar/borrar (confirm). Toast de exito/error. Nota al pie: los planes con promo se muestran solos.
- **Entry points (sin tocar el tabbar de 7 - decision Lote 34)**: en `GymShell.tsx` seccion **"Herramientas"** del sidebar desktop (`DesktopGymSidebar`) y del drawer movil (`GymDrawer`): nuevo link "Comunicados y Promos" con icono `Megaphone` (ember) a `/gimnasio/promos`. Ademas, card full-width en el panel principal `/gimnasio` (icono Megaphone ember + texto "Tus alumnos las ven en su app dentro de `Mi gimnasio`" + ChevronRight, Link a `/gimnasio/promos`).
- **Vista alumno `/mi-gimnasio`**: eliminada la seccion "Ultimos accesos" (y el estado `history`). Se agrego `owner_id` al embed de `gyms`. En "Dar el presente" se mantiene. Nuevas secciones (debajo):
  - **"Comunicados y promos"**: fetch `gym_announcements` por gym_id (limit 10, order desc) + `gym_plans` con `promo_type` no nulo + `getGymModules(gymId)` (para el toggle spotter_shop). Promos de planes: filas ember "2x1 · <plan> — $X · N meses". Publicaciones: badge COMUNICADO (neon) / PROMO (ember), titulo, texto, imagen, y si trae product_id un chip "Ligado" que linkea a `/market/[id]` mostrando nombre+precio (resuelto desde shopProducts, limit 12).
  - **"SpotterShop del gym"**: grid 3-4 cols de `market_products` activos con `seller_id = owner_id` (limit 12, muestra 8), tile imagen/proxy + nombre + precio link a `/market/[id]`, link "Ver tienda". Se **oculta** si el modulo `spotter_shop` esta OFF para el gym (consistencia Lote 33).
- Sin cambios en /checkin, membresia ni "Dar el presente". Lint 0 errores (8 warnings: 4 preexistentes + 4 nuevos del patron `<img>` ya aceptado en el repo) + build OK 45 rutas. Push deploy automatico.
- **PENDIENTE usuario**: correr `00042_gym_announcements.sql` en SQL Editor (con eso el Lote 35 queda CERRADO). **00042 CORRIDA por el usuario 25/09/2026 y verificada via REST**: SELECT a `gym_announcements` HTTP 200 (tabla existente, 0 filas) + INSERT anon rechazado con 401 (RLS funcionando). **Lote 35 CERRADO**.

**Lote 35b - /mi-gimnasio: solo "Articulos del gym" con expansion inline (26/09/2026, sin migracion, lint 0 errores, build OK 45 rutas):**
- El usuario pidio separar "promos de cuotas" y "articulos del gym" en 2 secciones, y al aclarar decidio: **sacar la seccion de promos/cuotas** (incluido el 2x1 automatico de los planes con promo, que lo habia confundido) y que queden **SOLO los articulos que vende el gym**, en cuadritos pequeños (2-3 filas) que al tocarlos **se expanden inline** para ver las fotos.
- **`/mi-gimnasio`**: eliminado el bloque "Comunicados y promos" (planes con `promo_type` + `gym_announcements` manuales) con sus estados e interfaces; la seccion "SpotterShop del gym" pasa a **"Articulos del gym"**. `ShopProduct` += `description`/`category` (select ampliado, limit 12). Nuevo estado `expandedId` + `photoIndex`: el tile tocado pasa a `col-span-full` (expansion inline, un solo expandido a la vez) y muestra **galeria de fotos** (imagen principal + miniaturas tappables si hay >1 imagen), nombre, precio neon, chip de categoria, descripcion y boton "Ver en SpotterShop" (`/market/[id]`); `ChevronUp` para cerrar. El gate `spotter_shop` (modulo) y "Ver tienda" intactos.
- El panel `/gimnasio/promos` y la tabla `gym_announcements` **no se tocan** (el gym puede seguir publicando; solo la vista del alumno muestra ahora unicamente los articulos). Sin migracion.

**Lote 35c - Panel del gym: "Articulos del gym" (publicar articulos, fin de comunicados/promos) (26/09/2026, sin migracion, lint 0 errores, build OK 45 rutas):**
- El usuario pidio que el panel del gym deje de publicar "Comunicados y Promos" (los alumnos ya no los ven) y publique **solo articulos** (los que se ven en `Mi gimnasio`).
- **`/gimnasio/articulos`** (NUEVA pagina, patron de `/market/crear` + `/market/mis-publicaciones`): form "Publicar articulo" (fotos max 5 subidas a `media/<uid>/market/<ts>.<ext>`, nombre*, descripcion, categoría chips `MARKET_CATEGORIES`, precio*, stock, condicion nuevo/usado, ubicacion) + banner `get_publish_quote` (publicacion gratis / "Articulo pago" con cuenta de cobro + contacto → `market_publish_pending` + toast con pedido). Lista **"Publicados"** con todos los estados (Publicado/Pausa/Vendido/Pendiente de pago), acciones **pausar/activar** (toggle status), **editar** (carga el form; reemplaza `UPDATE market_products` directo; fotos nuevas subidas, si no se eligen se conservan `existingImages`), **borrar** (confirm, RLS owner 00026), link a `/market/[id]`; realtime `market_products` filter `seller_id` (aparece solo cuando el admin aprueba un pendiente).
- **Eliminada** la pagina `/gimnasio/promos` (ruta `gym_announcements`). Table queda en la DB sin UI (inofensiva).
- **GymShell**: en las 2 secciones "Herramientas" (sidebar desktop + drawer movil) el link pasa a **"Artículos del gym"** (`ShoppingBag` neon) → `/gimnasio/articulos`; se elimina `Megaphone`.
- **`/gimnasio`**: la card "Comunicados y Promos" pasa a **"Artículos del gym"** (ShoppingBag neon) → `/gimnasio/articulos`.
- El item "SpotterShop" del nav GymShell (storefront) y la vista alumno `/mi-gimnasio` quedan intactos. Sin migracion.

**Lote 36 - Fix service role key (gym no podia crear miembros) + Crear cuenta de alumno desde 'Alumnos propios' (26/09/2026, sin migracion, lint 0 errores, build OK 46 rutas):**
- **Sintoma**: al agregar un miembro desde el panel del gym, Supabase respondia **403 'User not allowed'**. NO era la config de auth (verificada por GET a /auth/v1/settings: disable_signup=false, external.email=true, mailer_autoconfirm=true).
- **Causa raiz**: src/app/api/invite-member/route.ts resolvia la service role key con un **fallback silencioso a NEXT_PUBLIC_SUPABASE_ANON_KEY**. La clave se habia rotado en Supabase pero no estaba cargada en Vercel => caia a la anon key => uth.admin.createUser es rechazado por Supabase con 403 'User not allowed' (GoTrue exige un JWT service_role).
- **Fix (seguridad)**: nuevo helper src/lib/server/supabase-admin.ts con getServiceClient() + isServiceRoleConfigured() + MISSING_SERVICE_ROLE_ERROR. **Eliminado el fallback a la anon key**: si falta SUPABASE_SERVICE_ROLE_KEY (o SPOTTERX_SERVICE_ROLE) el endpoint responde 500 con un mensaje explicito en vez de un error opaco. invite-member/route.ts usa el helper.
- **REQUISITO DE CONFIG (el usuario lo tiene que hacer)**: generar/rotar la clave service_role en Supabase (Project Settings > API Keys) y cargarla en Vercel (proyecto spotterx > Settings > Environment Variables) como SUPABASE_SERVICE_ROLE_KEY en Production + Preview + Development, y **redesplegar** (las env vars solo aplican a despliegues nuevos). La clave NO se expone al cliente (sin prefijo NEXT_PUBLIC_).
- **Endpoint nuevo POST /api/trainer/students**: crea la cuenta de un alumno propio del profesor. Valida el Bearer token y que el caller sea **rol 'profesor' o admin** (server-side). Body: ull_name (nombre+apellido, requerido), email (opcional), username (opcional, se genera slug unico del nombre con sufijo aleatorio + reintentos). Crea el uth.users con clave provisional Spotter<6>! y email_confirm: true (mismo patron del gym), upserta/actualiza el profile (rol 'alumno') e inserta 	rainer_students(trainer_id=caller, source='propio', active:true) con upsert onConflict trainer_id,student_id -> **queda asignado a ese profesor**. **NO crea gym_memberships** (el alumno puede ser del gym con o sin membresia). Si el email ya existe en profiles, solo vincula la cuenta existente (no duplica) y responde existing: true.
- **UI /entrenamiento (tab 'Propios')**: boton **'Crear cuenta de alumno'** (visible solo si profiles.role==='profesor' o is_admin; la pagina antes no chequeaba rol). BottomSheet con nombre + apellido + email opcional. Al crear: toast + **card de credenciales** (email / clave / @usuario) con boton **'Copiar credenciales'** (
avigator.clipboard, patron del gym) y nota de que sin email se entra con el email provisorio y el alumno puede cambiarlo en Configuracion. Build 46 rutas (nuevo /api/trainer/students).
- **Robustez de la lista 'Propios'**: se elimin� el embed profile:student_id(...) (patron de embeds a profiles que falla en runtime, leccion mi-gimnasio/gimnasio-miembros); ahora consulta 	rainer_students + profiles por separado y une con un Map en el cliente. Ademas el select ahora filtra .eq('active', true) (antes mostraba vinculos dados de baja).
- **Decisiones del usuario**: el alumno se crea con **nombre y apellido suffice** (email opcional) y carga sus datos despues; la entrega de acceso es el **patron del gym** (clave provisional para copiar), NO un link de invitacion, porque el proyecto **no tiene SMTP** (no hay inviteUserByEmail ni generateLink en el repo, y el SMTP por defecto de Supabase solo envia a miembros de la organizacion). Solo el profesor ve el boton.
