...
### Fase A — Módulo Profesor: vista alumno + planes estructurados + rutinas fechadas (implementado; falta correr 00013)
...
- **Milestone 1 de Fase A**: A1 (ruta+rutinas+historial+chat), A2 (schema+UI profe+vista alumno, **sin plantillas**), A3 (due_on+completar+historial, **sin calendario ni gráfico**), A4 parcial (leídos marcados, seguir badge visible + adjuntos). Pendiente A5 (vinculación bidireccional + disponibilidad). Checkboxes en `PLAN_PULIDO.md`.
...
## Deploy / Producción
- **Vercel**: proyecto `pump13/spotterx` → producción `https://spotterx-five.vercel.app` (deploy **Fase A milestone 1 — mi-entrenamiento + planes estructurados** `https://spotterx-ieincs6ga-pump13.vercel.app`, 32s). Env vars de Supabase configuradas en production/preview/development. Redploy: `vercel --prod --yes` (requiere login o `VERCEL_TOKEN`). Token vigente transitorio (dura 1 día): `[REDACTED]`. (El token `[REDACTED]...` también perdió acceso al scope `pump13`; los tokens de Vercel son de corta duración y hay que regenerarlos.)
