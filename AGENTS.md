...
- **Migración `00021_foods_catalog.sql` PENDIENTE de correr**: tabla `foods` + RLS + unique `lower(name)` + realtime + seed de ~100 alimentos comunes argentinos. **Degrada con gracia**: sin la tabla, FoodPicker cae a input simple (igual que ExercisePicker).
