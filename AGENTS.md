...
### ✅ Etapa 3 — Gimnasio completo (implementado y deployado 12/09/2026); PENDIENTE usuario: migración 00011
- Migración **`00011_gym_reminders.sql`** (NUEVA, **pendiente de correr**): habilita tipo `vencimiento` en `notifications`, función `notify_upcoming_expiry()` (aviso 3 días antes del vencimiento de membresías activas y al día) y job **pg_cron** diario a las 09:00. Mientras no se corra, el resto de Etapa 3 funciona igual.
- **Cuenta de feed del gym** (decisión: el perfil del dueño ES el gym): pestaña **"Feed"** en el GymNav (`/perfil/<username del dueño>`, icono Sparkles), card **"Este gimnasio usa SpotterX"** en el perfil público de usuarios `role='gym'` (nombre + dirección/ciudad + mapa Leaflet, desde `gyms` por `owner_id`), y botón **"Publicar en el feed"** (`/crear`) en el panel.
...
- Commit `df88762`, deployado a **https://spotterx-five.vercel.app** (21s). Para completar la Etapa 3: correr `00011_gym_reminders.sql`.
...
## Deploy / Producción
- **Vercel**: proyecto `pump13/spotterx` → producción `https://spotterx-five.vercel.app` (deploy `https://spotterx-aejk06equ-pump13.vercel.app`). Env vars de Supabase configuradas en production/preview/development. Redploy: `vercel --prod --yes` (requiere login o `VERCEL_TOKEN`). Token vigente transitorio (dura 1 día): `[REDACTED]`. (El token `[REDACTED]...` perdió acceso al scope `pump13`; los tokens de Vercel son de corta duración y hay que regenerarlos.)
